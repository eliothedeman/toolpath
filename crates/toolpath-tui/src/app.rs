...
impl TraceTuiApp {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(crossterm::event::MouseButton::Left) => {
                ...
                if let Some((step_idx, sub)) = self.display_line_to_cursor(display_line) {
                    ...
                    if sub > 0 {
                        if let Some(fl) = self.get_field_line_at(step_idx, sub) {
                            if let Some(ptr) = fl.json_pointer.clone() {
            ...
            MouseEventKind::Drag(crossterm::event::MouseButton::Left) => {
                if let Some(ref drag) = self.mouse_drag.clone() {
                    ...
                    let display_line = self.scroll + row - 1;
                    if let Some((step_idx, sub)) = self.display_line_to_cursor(display_line) {
                        if step_idx == drag.step_index && sub > 0 {
                            if let Some(fl) = self.get_field_line_at(step_idx, sub) {
                                if fl.json_pointer.as_deref() == Some(&drag.json_pointer) {
                                    let char_offset = self.col_to_char_offset(col, &fl);
                                    self.selection = SelectionMode::Visual {
    ...
    fn visual_char_word_forward(&mut self) {
        ...
        {
            if let Some(value) = self.get_field_value(&step_index, &json_pointer) {
